...
function getOrderList(pageNum,pageSize){
	...
	pairequest("/pai/orderform/orderformlist.do",reqdata).then(function(data){
...
//							if(order.freezeflag=="0" || order.freezeflag==null || orderfreezeflag==undefined){
//								orderOperate="<span class='colorgreen' style='cursor:default' onclick='getMoney("+order.orderNo+")'>解冻资金</span><br>";
//							}else{
...
//							orderOperate="<span class='layui-btn layui-btn-sm layui-btn-radius layui-btn-warm'  onclick='viewCheckorder(&apos;"+order.orderNo+"&apos;)'>查看对账单</span><br>";
						...
						htmlstr=htmlstr+"<li>"+
							...
							+"<div class='yzs-m-order-list-con2'>"
								...
								+orderOperate
								+"</div>"
								+"<div class='yzs-m-order-list-opt'><a href='#' class=''><a target='_blank' href='/orderform/sellerordershow.html?param="+order.orderNo+"' class='layui-btn layui-btn-radius layui-btn-sm' >订单详情</a></div>"
							+"</div>"
						+"</li>"
					}
					$("#orders").html(htmlstr);
...
function sign(orderNo){
	...
	pairequest("/pai/orderform/getSignParam.do",reqdata).then(function(data){
		console.log(data);
		if(data.success==true){
			var jsondata={};
...
function   addoderid(jsondata){
	...
	pairequest("/pai/processtemplent/getTemplentparams.do",reqdata).then(function(data){
...
function generateCheckorder(orderNo){
	...
	layer.open({
		 anim: 2,
		   ...
		   shadeClose: false,
	      content: ['/orderform/checkorder/seller_checkorder_add.html?v='+new Date().getTime(), 'yes'],
...
function editCheckorder(orderNo){
	...
	layer.open({
		   ...
		   skin: 'layui-layer-nobg', //没有背景色
		   shadeClose: false,
	      content: ['/orderform/checkorder/seller_checkorder_edit.html?v='+new Date().getTime(), 'yes'],
	      success:function(layero, index){
	    	var body = layer.getChildFrame('body', index);
	        var iframeWin = window[layero.find('iframe')[0]['name']];
	        body.find('#orderNo').val(orderNo);
...
function getMoney(orderNo){
	...
	pairequest("/pai/orderform/checkorder/checkneedpay.do",reqdata).then(function(data){
		...
		if(data.success==true){
			...
			if(status==0){
			    ...
			    layer.confirm(str, {btn: ['确定', '取消'], title: "提示"},  function () {
					...
					topay("/pai/orderform/checkorder/payservicecharge.do",param);
			    });
			}else if(status==3){
			    ...
			    layer.confirm(
			    	...
			    	function () {
				    	var param={};
						param["orderNo"]=orderNo;
						topay("/pai/orderform/checkorder/getMoney.do",param);
				    },
				    function(){
				    	var param={};
						param["orderNo"]=orderNo;
				...
				//直接收款
				$.ajax({
					url:"/pai/orderform/checkorder/getMoney.do",
					type:"get",
					data:{
						'orderNo':orderNo
					},
...
function finishOrder(orderNo){
	...
	pairequest("/pai/orderform/finishOrder.do",reqdata).then(function(data){
		...
		if(data.success==true){
			...
			if(obj && obj.code=="01"){
				var status=data.obj.status;
				if(status==0){
					var str = "<div><h4>您需要先支付完成服务费</h4><p>金额："+obj.servicecharge+"元</p></div>";
					layer.confirm(str, {btn: ['确定', '取消'], title: "提示",closeBtn:false}, function () {
						var param={};
						param["orderNo"]=orderNo;
						topay("/pai/orderform/checkorder/payservicecharge.do",param);
					},function(){
						console.log(obj);
						$.ajax({
							url:"/pai/orderform/checkorder/seller/cancelcheckorder.do",
							type:"get",
							data:{
								'ckid':obj.ckid,
							},
...
function importantnotice(){
	setTimeout(function() {
		layer.tips('重要提示：订单自竞价成功之日起30日内若无任何对账交易，系统将自动按比例收取交易服务费。详情请见<a href="/bank/monthfee.html?v=20191024" target="_blank">手续费中心</a>。', '.important_notice', {
			  tips: [1, '#FF5722'],
			  time: 30000
			});
	}, 2000);
	...
	setTimeout(function() {
		layer.tips('重要提示：订单自竞价成功之日起30日内若无任何对账交易，系统将自动按比例收取交易服务费。详情请见<a href="/bank/monthfee.html?v=20191024" target="_blank">手续费中心</a>。', '.toOpenMem', {
			  tips: [1, '#FF5722'],
			  time: 30000
			});
	}, 1000);
