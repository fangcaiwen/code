		...
		if(data.total==0 || data.total == "0"){
            ...
            if(data.content != null){
                ...
                $.each(data.content, function(index, value) {
                    ...
                    result += '<span class="ti '+value.tid+'ti">进行中</span>';
                    if(value.goodsname.split(",").length>1){
                   	 result += '<span class="tig">集采</span>';
                    }
            	    result += '<div class="img"><img src="'+value.url+'" alt="" srcset=""></div>';
//                    if(checkSign(value.opentendertime)){
                    	 result += '<div class="datetime '+value.tid+'datetime"><i></i><span class="tim '+value.tid+'">开标时间：'+value.opentendertime.substring(0,19)+'</span></div>';
//                    }
                    result += '<div class="wrap">';
                    result += '<div class="nameAddress">';
                    result += '<div class="name">'+value.tname.substring(0,10)+'...</div>';
